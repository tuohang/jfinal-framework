...
public class AutoTableBindPlugin extends ActiveRecordPlugin {
	...
	private String libDir = PathKit.getWebRootPath() + File.separator
	...
	public AutoTableBindPlugin(IDataSourceProvider dataSourceProvider) {
		this(DbKit.MAIN_CONFIG_NAME, dataSourceProvider,
				SimpleNameStyles.DEFAULT);
	}

	public AutoTableBindPlugin(String configName,
	...
	public AutoTableBindPlugin(String configName,
		...
		this(configName, dataSourceProvider, transactionLevel,
				SimpleNameStyles.DEFAULT);
	}

	public AutoTableBindPlugin(IDataSourceProvider dataSourceProvider,
			INameStyle nameStyle) {
		super(DbKit.MAIN_CONFIG_NAME, dataSourceProvider);
		this.nameStyle = nameStyle;
	}

	public AutoTableBindPlugin(String configName,
			IDataSourceProvider dataSourceProvider, INameStyle nameStyle) {
		super(configName, dataSourceProvider);
		this.nameStyle = nameStyle;
	}

	public AutoTableBindPlugin(IDataSourceProvider dataSourceProvider,
			int transactionLevel, INameStyle nameStyle) {
	...
	@SuppressWarnings({ "rawtypes" })
	public AutoTableBindPlugin addExcludeClasses(
			Class<? extends Model>... clazzes) {
	...
	@SuppressWarnings("rawtypes")
	public AutoTableBindPlugin addExcludeClasses(
			List<Class<? extends Model>> clazzes) {
		if (clazzes != null) {
	...
	public boolean start() {
		List<Class<? extends Model>> modelClasses = ClassSearcher
				.of(Model.class).libDir(libDir).classpath(classpath)
				.scanPackages(scanPackages).injars(includeJars)
		...
		for (Class modelClass : modelClasses) {
			...
			if (tb == null) {
				...
				log.debug(arpConfName + " addMapping(" + tableName + ", "
						+ modelClass.getName() + ")");
			} else {
				String tbConfName = tb.configName();
				if (StrKit.notBlank(tbConfName)
						&& !tbConfName.equals(arpConfName))
					continue;
				// 如果tableName注解为空，默认为model名小写
				if (StrKit.notBlank(tb.value()))
					tableName = tb.value();
				else
					tableName = nameStyle.name(modelClass.getSimpleName());
